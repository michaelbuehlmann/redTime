...
private: /////////////////////////////////////////////////////////////////////
  ...
  static constexpr double C_nu_hot =
      0.681321952980717; // 3.0*(7.0/8.0) * pow(4.0/\11.0,4.0/3.0);
  ...
  int input_data_init(input_data *dat0) {
    ...
    for (int j = 0; j < n_interp_z; j++)
      dat0->zT[j] = z_transfer_str[j];
    return 0;
  }
  ...
  static int func_to_integrate_d_dDda_kdep(double a, const double y[],
    ...
    double F_MG =
        2.0 * k0 * k0 * beta_mg * beta_mg / (k0 * k0 + a * a * m_mg_a * m_mg_a);
  ...
  static int integrate_growth(double a_begin, double a_end, input_data *d,
                              double *y) {
    ...
    gsl_odeiv_system sys = {func_to_integrate_d_dDda_kdep, dummy_jacobian, 2,
                            d};
  ...
  static double Plin_integrand_unnorm(double lnkR, void *params) {
    ...
    dat->p[10] = k;
    double T = Transfer_cb(*dat),
           F = 1.0 - dat->p[5] / dat->p[3] + Beta_P(1, *dat);
    double W = 1.0 - 0.1 * kR * kR;
    if (kR > 1e-2)
      W = 3.0 * (sin(kR) / kR3 - cos(kR) / kR2);
    return W * W * T * T * F * F * pow(k, dat->p[0] + 3.0) /
           (2.0 * M_PI * M_PI);
  }
...
public: ///////////////////////////////////////////////////////////////////////
  ...
  cosmological_parameters(string paramfile_inp) {
    ...
    if (DEBUG_INPUT)
      std::cout << "cosmological_parameters: read z_in=" << z_in_inp
                << std::endl;
    ...
    input >> tc_c;
    if (DEBUG_INPUT)
      std::cout << "cosmological_parameters: transfer file: " << tc_c.c_str()
                << std::endl;

    int neut_interp_type = -100;
    cosmological_parameters::discard_comments(&input);
    input >> neut_interp_type;
    if (neut_interp_type != 0)
      abort(); // ONLY DO THIS CASE SO FAR
    ...
    if (n_interp_z > MAXTRANSFER) {
      std::cout << "ERROR: Number of transfer function inputs (" << n_interp_z
                << ") exceeds MAXTRANSFER=" << MAXTRANSFER
  ...
  int transferfile(char *tfile) {
    strcpy(tfile, tc_c.c_str());
    return 0;
  }
  ...
  static double E(double a, double *p) {
    return std::pow(a, -3.0 * (1.0 + p[8] + p[9])) *
           exp(-3.0 * p[9] * (1.0 - a));
  }
  ...
  double dlnH_dlna(double a) {
    return 0.5 * a / H2_H02(a) *
           (fcb_c * Om_c * (-3.0 * (1.0 + Y(a)) + a * dYda(a)) /
                std::pow(a, 4) +
            OL_c * dEda(a) - 4.0 * Og_c / std::pow(a, 5));
  }
  static double dlnH_dlna(double a, double *p) {
    double Og = C_rho_gam * std::pow(p[7] * p[7] / p[2], 2);
    double fn = p[5] / p[3], fc = 1.0 - fn;
    return 0.5 * a / H2_H02(a, p) *
           (fc * p[3] * (-3.0 * (1.0 + Y(a, p)) + a * dYda(a, p)) /
                std::pow(a, 4) +
      ...
      for (int i = 1; i < dat.n_z; i++) {
        ...
        do {
          ...
          if (status) {
            ...
            if (DEBUG_INTERP)
              std::cout << "#Beta_P: " << filename << setw(20) << aArr[i]
                        << setw(20) << kArr[j] << setw(20)
                        << BetaArr[i * n_k + j] << std::endl;
          }
        } while (status && ++j < n_k);
  ...
  /* for a given redshift z*/
    ...
    static tabulated_function G_lna_lnk(n_lna + 1,
                                        n_lnk + 1); // G(ln a, ln k) = D/a
    static tabulated_function dDda_lna_lnk(n_lna + 1,
                                           n_lnk + 1); // dD/da(ln a,ln k)
    ...
    if (!init) {
      ...
      for (int i = 0; i <= n_lna; i++)
        lnaTab[i] = lna_min + dlna * i;
      for (int j = 0; j <= n_lnk; j++)
        lnkTab[j] = lnk_min + dlnk * j;
...
#pragma omp parallel for schedule(dynamic)
      ...
      for (int j = 0; j <= n_lnk; j++)
        DnormTab[j] = G_lna_lnk(0, lnkTab[j]);
  ...
  int H0chi_eta_init(void) {
    ...
    double zmin = 1e-4, zmax = 1e4, dlnz = log(zmax / zmin) / (N_H0CHI - 1),
           zlast = 0;
    input_data dat;
    input_data_init(&dat);
    ...
    for (int i = 0; i < N_H0CHI; i++) {
      ...
      zlast = z;
      H0chi_i[N_H0CHI - 1 - i] =
          (i == 0 ? DH0chi : DH0chi + H0chi_i[N_H0CHI - i]);
    }
  ...
  static double Transfer_cb(const input_data dat) {
    ...
    if (!init) {
      ...
      for (int i = 0; i < nVars; i++)
        status = status && transfer_input >> temp[i];
      while (status) {
        discard_comments(&transfer_input);
        kTi[nTk] = temp[i_k];
    ...
    if (DEBUG_LINEAR)
      cout << "#Transfer end. returning T=" << exp(log_transfer(log(dat.p[10])))
           << endl;
  ...
  static double Plin(double z, input_data dat0) {
    ...
    if (DEBUG_LINEAR)
      cout << "#Plin end. Returning Plin="
           << Norm * pow(dat0.p[10], dat0.p[0]) * T * T * DdD[0] * DdD[0]
           << endl;
  ...
  double sigmaV2(double z) {
    ...
    if (!init) {
      ...
      gsl_integration_qag(&F, x0, x1, epsabs, epsrel, ws_size, int_type, w,
                          &result, &error);
