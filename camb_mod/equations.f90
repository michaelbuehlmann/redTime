...
!            optimized neutrino sampling, and reorganised neutrino integration functions
	 ...
	 real(dl) :: w_lamprime = 0.0_dl
         ...
         logical :: DynamicalDarkEnergy = .false. !!!RBADD


	contains 

	function w_lama(a)	!!!RBADD 
	
	double precision:: w_lama 
	double precision :: a
	w_lama = w_lam +(a - 1)*w_lamprime  
	end function w_lama !!!!RBADD

	function w_eff(a) !!!RBADD

		double precision:: w_eff, a 
		double precision ::temp

		temp=w_lam - w_lamprime
		temp= temp - (1-a)*w_lamprime/DLog(a) 
		w_eff =temp
	end function w_eff!!!RBADD


       subroutine SetDarkEnergyType
!Set the logical variable DynamicalDarkEnergy in module LambdaGeneral
!to decide which sort of perturbation equations to use for dark energy.
!
!R. Biswas, June 2008
!
![/desc]

	if (w_lam /= 1.d0) DynamicalDarkEnergy = .true.
	if (w_lamprime /= 0.d0) DynamicalDarkEnergy = .true.
	if (cs2_lam /= 1.d0) DynamicalDarkEnergy = .true.
	if (w_perturb .eqv. .true.) DynamicalDarkEnergy = .true.
	end subroutine SetDarkEnergyType
...
!Background evolution
        function dtauda(a)
         !get d tau / d a
	!Modified to allow for a w_0, w_a model
	!Modified to exit gracefully if timeofz fails 
        use precision
        use ModelParams
        use MassiveNu
        use LambdaGeneral
        ...
        integer nu_i
if (TimeOfz_Fails) then !RBCHECKLATER
    TimeOfz_Fails=.false.
    write (*,*) w_lam, w_lamprime
end if
...
!  8*pi*G*rho*a**4.
        ...
        !if (w_lam == -1._dl) then			!RBCOMM
        !   grhoa2=grhoa2+grhov*a2**2			!RBCOMM
        ! else						!RBCOMM
        !   grhoa2=grhoa2+grhov*a**(1-3*w_lam)		!RBCOMM
        ! end if					!RBCOMM
	grhoa2=grhoa2+grhov*a**(1-3*w_eff(a))
...
function HdotoH(a)
!Function to get 
!S \equiv \frac{\dot{H}}{H} (a)
!the derivative of the Hubble parameter wrt physical 
!time divided by itself. 
...
real(dl), intent(IN) :: a
real(dl) dtauda
real(dl) rhonu,grhoa2, a2 , gpa2,HdotoH,S
integer nu_i
external dtauda

if (TimeOfz_Fails) then
    TimeOfz_Fails=.false.
    write (*,*) w_lam, w_lamprime
end if
...
a2=a**2
grhoa2=grhok*a2+(grhoc+grhob)*a+grhog+grhornomass
grhoa2=grhoa2+grhov*a**(1-3*w_eff(a))!RBADD
...
!Get massive neutrino density relative to massless
        ...
        !!!Now calculate pressure 8*pi*G*p*a**4  
gpa2 = grhok*a2*(-1.d0/3.d0) + (grhog + grhornomass)*(1.d0/3.d0)
        !!!the term for baryons and cdm drops out as assumed to be cold
gpa2=gpa2+grhov*a**(1-3*w_eff(a))*w_lama(a) !RBADD

        !!!Set pressure of massive neutrino =0 
if (CP%Num_Nu_massive /= 0) then
                !Get massive neutrino density relative to massless
        do nu_i = 1, CP%nu_mass_eigenstates
        call Nu_rho(a*nu_masses(nu_i),rhonu)
        gpa2=grhoa2 + 0.d0 !rhonu*grhormass(nu_i)
    end do
end if
        !!!Need -4*pi*G*(rho +P)
S = (grhoa2 + gpa2)/(a2**2.0d0)/2.0d0
HdotoH = S*a2*dtauda(a)

!write (*,*) HdotoH*(1./10**9.d0)
end function HdotoH

function S(a)
![desc]
!Function to get 
!S \equiv \frac{H^{\prime}}{H} (a)
!where H^{\prime} is the hubble parameter at a while H is the hubble
!parameter if it is radiation dominated.
!
!R. Biswas Sep, 2008
![/desc]
...
!  Compute expansion rate from: grho 8*pi*rho*a**2
        ...
        grhog_t=grhog/a2
		!!!expansion rate uses the	!RBCOMMENT 
		!!!integrated w value		!RBCOMMENT
        !grhov_t=grhov*a**(-1-3*w_lam)
	grhov_t=grhov*a**(-1-3*w_eff(a))	!RBADD
        grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t
		!!!pressure term depends on the	!RBCOMMENT 
		!!! w at that time		!RBCOMMENT
        !gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
	gpres=(grhog_t+grhor_t)/3+grhov_t*w_lama(a) !RBADD
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
		...
		!RBCHECKLATER
           dgq = dgq + vq*grhov_t*(1+w_lama(a)) !RBADD
...
!  Internal variables.
	...
	real(dl) frame_epsilon! Smoothing RBADD  
	
	frame_epsilon = 0.00001 !Smoothing value
...
if (w_lam /= -1 .and. w_Perturb) then
		...
		!RBCOMMENT: Density perturbation equation has no issues
		!RBCOMMENT: change w_lam to time dependent w
		!RBCOMMENT: add wdot term which is otherwise absent
	ayprime(EV%w_ix)= &					!RBAADD
		-3*adotoa*(cs2_lam-w_lama(a))*(clxq+3*adotoa*(1+w_lama(a))*vq/k)&
		-(1+w_lama(a))*k*vq -(1+w_lama(a))*k*z & 	!RBADD
		-3*adotoa*adotoa*a*w_lamprime*vq/k 		!RBADD 		
		!RBCOMMENT: velocity pertrurbation has issues crossing w = -1
           !ayprime(EV%w_ix+1) = -adotoa*(1-3*cs2_lam)*vq + k*cs2_lam*clxq/(1+w_lam)!RBCOMMENT: This was true for normal CAMB
	if (abs(1.0d0 +w_lama(a)).gt. frame_epsilon ) then
			!!!RBCOMMENT: no crossing -1, no fudge
			!!!RBCOMMENT: Just change w for time dependence
		ayprime(EV%w_ix+1) = &			!RBADD
			-adotoa*(1-3*cs2_lam)*vq &	!RBADD
			+ k*cs2_lam*clxq/(1+w_lama(a))	!RBADD
	else
			!!!RBCOMMENT: now w crosses -1
			!!!RBCOMMENT: fudge, so that this derivative
			!!!	matches at boundary = frame_epsilon
			!!!	and goes to 0 at crossing 
		ayprime(EV%w_ix+1) = &
			-adotoa*(1-3*cs2_lam)*vq + &			 !RBADD
			k*cs2_lam*clxq*(1+w_lama(a))/frame_epsilon**2.0d0!RBADD
	end if 		
...
!  Use explicit equation for vb if appropriate
		...
		gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lama(a)!RB Bug removed
                                                        !Nov 2008
