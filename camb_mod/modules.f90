...
!     for restrictions on the modification and distribution of this software.
        ...
        integer, parameter :: max_transfer_redshifts = 100    
...
! Main parameters type
        type CAMBparams
         ...
         integer   :: NonLinear
         integer   :: thismodel !!! refers to Super Coyote model number
...
!     lmax is max possible number of l's evaluated
	...
	!fiducial high-accuracy high-L C_L used for making small cosmology-independent numerical corrections
      ...
      logical :: DeltaTime_Fails = .false.			!RBADD
      logical :: TimeOfz_Fails = .false.			!RBADD
      logical :: AngularDiameterDistance_Fails = .false. 	!RBADD
      contains
...
!           GetTestTime = etime(tarray)
	...
	character (len=30) ::sname='function DeltaTime'!RBADD
		...
		if (rombint_fails) then		!RBADD 
			!!!since we know what fails		!RBCOMMENT 
			!!!now, tag the failing function 	!RBCOMMENT
			!!!and reset the rombint_fails variable	!RBCOMMENT
		    rombint_fails = .false.	!RBADD	
                    write (*,*)	'rombint call from DeltaTime fails for'
                    write (*,*) a1, a2, atol 
                    DeltaTime_Fails =.true.
                end if 
	...
	if (DeltaTime_Fails) then
		DeltaTime_Fails=.false.
		TimeOfz_Fails=.true.
              	write (*,*) 'DeltaTime call from TimeOfz fails from', z
                write (*,*) CP%omegab, CP%omegac, CP%omegav, CP%H0
                !write (*,*) dtauda(1.0d0)
	end if !RBADD
	...
	if (DeltaTime_Fails) then
		DeltaTime_Fails =.false.
		write (*,*) "DeltaTime call fails from Angular" ,&
			" DiameterDistance at redshift ",z
			AngularDiameterDistance_Fails =.true.
	end if
