...
public:
  tabulated_function(char *filename, int nCol, const int nvars, const int nrx,
                     const int nrf) {
    ...
    do {
      ...
      for (int i = 0; i < nvars; i++)
        status = status && (tableFile >> temp_in[i]);
  ...
  tabulated_function(int nColx, int nColy, const double *xArray,
    ...
    if (TF_DEBUG_ALLOC)
      std::cout << "#tabulated_function: constructor3: allocated x, y, and f "
                   "arrays of size "
    ...
    for (int i = 0; i < tableSizeY; i++)
      yTable[i] = yArray[i];
    for (int i = 0; i < tableSizeX * tableSizeY; i++)
      fTable[i] = fArray[i];
  }
  ...
  int initialize(int nColx, int nColy, const double *xArray,
    ...
    if (TF_DEBUG_ALLOC)
      std::cout << "#tabulated_function: initialize3: allocated x, y, and f "
                   "arrays of size "
                << tableSizeX << " " << tableSizeY << std::endl;

    if (nColx == 0 || nColy == 0) {
      std::cout << "invalid table: nColx=" << nColx << ", nColy=" << nColy
                << std::endl;
      std::abort();
    }
    // fill tables from inputs
  ...
  double f(double x) const {
    ...
    if (n1 <= 0) // allows for extrapolation to the left
      ...
      return tabulated_function::cubicInterp(&xTable[n1 - 1], &fTable[n1 - 1],
                                             x);
  }
  ...
  double f(double x, double y) const {
    ...
    if (ny < 0 || ny >= tableSizeY) {
      ...
      std::cout << "#      tableSizeX=" << tableSizeX
                << ", tableSizeY=" << tableSizeY << std::endl;
      std::abort();
    }
...
private:
  ...
  inline double fmax(double *x, int len) const {
    return ((len == 2) ? (fmax(x[0], x[1]))
                       : (fmax(fmax(x, len - 1), x[len - 1])));
  }
  inline double fmin(double *x, int len) const {
    return ((len == 2) ? (fmin(x[0], x[1]))
                       : (fmin(fmin(x, len - 1), x[len - 1])));
  }
  ...
  double linInterp(double *xTab, double *fTab, double xEval) const {
    double F =
        fTab[0] + (fTab[1] - fTab[0]) / (xTab[1] - xTab[0]) * (xEval - xTab[0]);
    return F;
  }
  ...
  double cubicInterp(double *xTab, double *fTab, double xEval) const {
    ...
    if (xEval < fmin(xTab, 4) || xEval > fmax(xTab, 4)) {
      std::cout << "cubicInterp WARNING: xEval=" << xEval
                << " out of bounds.  You are" << std::endl
    ...
    double F = (xEval - xTab[1]) * (xEval - xTab[2]) * (xEval - xTab[3]) /
                   (xTab[0] - xTab[1]) / (xTab[0] - xTab[2]) /
                   (xTab[0] - xTab[3]) * fTab[0] +
               (xEval - xTab[0]) * (xEval - xTab[2]) * (xEval - xTab[3]) /
                   (xTab[1] - xTab[0]) / (xTab[1] - xTab[2]) /
                   (xTab[1] - xTab[3]) * fTab[1] +
               (xEval - xTab[0]) * (xEval - xTab[1]) * (xEval - xTab[3]) /
                   (xTab[2] - xTab[0]) / (xTab[2] - xTab[1]) /
                   (xTab[2] - xTab[3]) * fTab[2] +
               (xEval - xTab[0]) * (xEval - xTab[1]) * (xEval - xTab[2]) /
                   (xTab[3] - xTab[0]) / (xTab[3] - xTab[1]) /
                   (xTab[3] - xTab[2]) * fTab[3];
    return (F);
  }
  ...
  int findN(double x) const {
    ...
    int n = 0;
    while (xTable[n + 1] < x && n < tableSize - 2) {
      n++;
    }
    return n;
  }
  ...
  int findNx_2d(double x) const {
    ...
    // large tables are necessary
    int n = 0;
    while (xTable[n + 1] < x && n < tableSizeX - 2) {
      n++;
    }
    return n;
  }
  ...
  int findNy_2d(double y) const {
    ...
    // large tables are necessary
    int n = 0;
    while (yTable[n + 1] < y && n < tableSizeY - 2) {
      n++;
    }
    return n;
  }
