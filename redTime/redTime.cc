...
#pragma omp parallel for schedule(dynamic)
  for(int i=0; i<nk; i++){
    ...
    if(dy[2*nk+i] < -10.0) dy[2*nk+i] = -10.0;
    if(dy[2*nk+i] >  10.0) dy[2*nk+i] =  10.0;
...
#pragma omp parallel for schedule(dynamic)
  ...
  for(int i=nUP*nk; i<nU*nk; i++) y[i] = 0.0;
  ...
  for(int i_eta=0; i_eta<C.n_eta(); i_eta++){
	...
	dlnB_dlna = (C.f_nu() < 1e-10 ? 0
      ...
      if(C.PRINTLIN())
	cout << setw(WIDTH) << D[0]
	     << setw(WIDTH) << C.asteps(i_eta)*D[1]/D[0]
	     << setw(WIDTH) << C.Plin_cb(C.zsteps(i_eta),kArr[i])
	     << setw(WIDTH) << B_eta / (B1 + 1e-100)
	     << setw(WIDTH) << dlnB_dlna
	     << setw(WIDTH) << C.Plin_nu(C.zsteps(i_eta),kArr[i]);
